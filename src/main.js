...
// 158 lookups that used to inline here now centralise behind one
// factory. The shape of `els` is unchanged.
...
// Two cross-cutting helpers wrap the legacy inline behaviour for
// 'toggle-menu' and 'copy-message' so the registry config can stay
// purely declarative.
function handleToggleMenu(target) {
  const nodeId = target.dataset.nodeId;
  activeRoundtableMessageId = null;
  activeMenuNodeId = activeMenuNodeId === nodeId ? null : nodeId;
  renderMenu();
}
function copyMessageNodeText(nodeId) {
...
const handleCommand = createCommandRegistry(buildCommandMap({
  ...
  renameCreatorMemory,
  queryCreatorMemory,
  clearCreatorMemoryLookup,
  clearCreatorRecords,
  openCreatorRecordDetail,
  openCreatorMemoryDetail,
  deleteCreatorRecord,
  ...
  importRoundtablePersonas,
  exportRoundtablePersonas,
  importRoundtableMemberFromSession,
  sendAssistantPrivateChat,
  togglePrimaryRoundtableSpeaking,
  toggleRoundtableMember,
  moveRoundtableMember,
  selectSealedCreator,
  toggleRoundtableRound,
  startRoundtableRound,
  resumeRoundtableRound,
  stopRoundtableGeneration,
  insertRoundtableMention,
  ...
  toggleSettingsModelPicker,
  selectSettingsModelFromPicker,
  toggleAssistantModelPicker,
  selectAssistantModelFromPicker,
  saveNovel,
  ...
  importBodyFile,
  exportBodyFile,
  syncBodyFromAssistant,
  generateNovelMaterial,
  applyLayoutPreset,
  ...
  copyLayoutParams,
  resetLayoutParams,
  toggleRoundtableMenu,
  copyRoundtableMessage,
